        ...
        return [
            {
                ...
                "logic": "先比较短均线与长均线的相对位置，再结合量能、波动和价格伸展过滤，只有趋势确认后才给出买入或减仓方向。",
            ...
            },
            {
                ...
                "logic": "用 RSI 识别超买超卖区，再用布林带判断价格是否偏离均值，重点观察超跌反弹或高位回落后的回归机会。",
            ...
            },
            {
                ...
                "logic": "围绕 DIF/DEA 金叉死叉和柱状图扩张收缩判断动量是否继续增强，并用经理式过滤减少零轴附近的噪音信号。",
            ...
            },
            {
                ...
                "logic": "它不会真正启动独立网格执行引擎，而是借用现有双均线表单做研究占位，用来记录区间震荡假设、仓位边界和后续实现方向。",
            ...
            },
            {
                ...
                "logic": "把多个子策略的方向、强弱和置信度按权重合并，冲突明显时自动观望，重点用于观察哪些因子在当前样本里真正起作用。",
            ...
            },
            {
                ...
                "logic": "以 RL 基线或训练模型的目标仓位输出为观察对象，先验证仓位切换逻辑和最低置信度门槛，再决定是否值得继续做组合层实现。",
                ...
                "research_only": True,
                "parameter_bounds": {
                    **self._parameter_bounds(list(rl_meta["parameter_schema"])),
                    "min_confidence": {
                        **self._parameter_bounds(list(rl_meta["parameter_schema"])).get("min_confidence", {}),
                        "default": 0.45,
                    },
                },
                "payload": {
                    ...
                    "execution_mode": "signal_only",
                    "parameters": {"rl_policy_mode": "baseline", "max_position_pct": 0.5, "min_confidence": 0.45},
                },
            },
        ]
