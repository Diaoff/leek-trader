...
class SmartSelectionRun(Base):
    ...
    progress_step: Mapped[int] = mapped_column(Integer, default=0, comment="当前执行步骤")
    progress_total: Mapped[int] = mapped_column(Integer, default=0, comment="执行总步骤")
    progress_label: Mapped[str | None] = mapped_column(String(64), nullable=True, comment="当前执行阶段")
