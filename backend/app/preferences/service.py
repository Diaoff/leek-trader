...
class PreferenceService:
    ...
    def get_preferences(self, db: Session, tenant_id: str = settings.default_tenant_id) -> PreferencesRead:
        preference = self._ensure_preferences(db, tenant_id)
        return self._serialize(db, preference, tenant_id)
    ...
    def update_preferences(
        ...
        if payload.smart_selection is not None:
            self._update_smart_selection(db, tenant_id, payload.smart_selection)
    ...
    def trading_preferences(self, preference: AppPreference | None = None, db: Session | None = None) -> TradingPreferences:
        if preference is None:
            if db is None:
                return TradingPreferences()
            preference = self._ensure_preferences(db, settings.default_tenant_id)
    ...
    def _ensure_preferences(self, db: Session, tenant_id: str) -> AppPreference:
        preference = db.scalar(select(AppPreference).where(AppPreference.tenant_id == tenant_id))
        if preference is None:
    ...
    def _serialize(self, db: Session, preference: AppPreference, tenant_id: str) -> PreferencesRead:
        smart_selection_config = self.smart_selection_service.get_config(db, tenant_id)
    ...
    def _update_smart_selection(
        ...
        current = self.smart_selection_service.get_config(db, tenant_id)
